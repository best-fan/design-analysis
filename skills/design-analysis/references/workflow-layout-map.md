...
- **.pen（Pencil 设计稿）**：
  ...
  - 可用工具：`mcp__pencil__open_document`、`mcp__pencil__snapshot_layout`、`mcp__pencil__get_screenshot`、`mcp__pencil__batch_get` 等
  - **必须先使用 `mcp__pencil__open_document` 打开 .pen 文件**
  - 确认顶层 frame 及多状态（如有）

- **Figma 链接**：
  - 使用 **Figma MCP** 从链接中解析 file key / node id
  - 可用工具：`mcp__Framelink_Figma_MCP__download_figma_images`、`mcp__Framelink_Figma_MCP__get_figma_data` 等
  - 获取对应 frame 或节点的截图、样式、布局与节点信息

- **MasterGo 链接**：
  - 使用 **MasterGo MCP** 从链接中解析 file key / layer_id
  - 可用工具：`mcp__mastergo-magic-mcp__mcp__getMeta`、`mcp__mastergo-magic-mcp__mcp__getDsl` 等
  - 获取对应 页面的元数据信息和规则 或节点的样式、布局与节点信息
...
1. 获取设计稿根节点
2. 遍历根节点的所有直接子节点
3. 过滤出具有实际尺寸的 GROUP/FRAME/INSTANCE 节点
4. 统计总数并记录
5. 按 y 坐标排序后逐一命名
```
...
为避免手动统计遗漏，推荐使用配套分析脚本：

| 设计稿类型 | 脚本路径 | 用途 |
|------------|----------|------|
| MasterGo | `scripts/mastergo-dsl-analyzer.py` | 统计主要区域、提取文字和样式 |
...
# 输出 Markdown 校验报告
python scripts/mastergo-dsl-analyzer.py <dsl_json_file> --format markdown --url "设计稿链接" --name "模块名称"

# 保存到文件
python scripts/mastergo-dsl-analyzer.py <dsl_json_file> --output report.md --format markdown
```
...
| 场景 | 问题原因 | 解决方法 |
|------|----------|----------|
| **底部区域遗漏** | y > 1080（超出首屏），未完整扫描 | 必须检查所有 y 坐标，包括 y > 页面高度 50% 的区域 |
| **宽区域遗漏** | 区域宽度接近页面宽度，被误判为"容器" | 不要过滤接近页面宽度的区域，所有直接子节点都应统计 |
| **脚本过滤错误** | 自编脚本过滤条件不当 | 使用本技能提供的官方脚本 |
...
- 再在同一行内按 x 从左到右读取
- 使用 Pencil MCP 工具（`mcp__pencil__snapshot_layout`、`mcp__pencil__batch_get` 等）时也按此顺序逐层获取布局与节点信息
